...
spikeRange = RelaxTime{5} >= 177.987 & RelaxTime{5} <= 186.366;

% Exclude spike data
RelaxTime_clean = RelaxTime{5}(~spikeRange); % x-values without the spike
CS_clean = CStress{5}(~spikeRange);  % y-values without the spike

% Interpolate to fill the missing values
RelaxTime_spike = RelaxTime{5}(spikeRange); % x-values in the spike range
...
CStress{5}(spikeRange) = CS_interpolated;
...
for i = 1:numFiles
    ...
    if ~isempty(RelaxTime{i})
        ...
        plot(RelaxTime{i}(validIndices), CStress{i}(validIndices)*1e3, 'LineWidth', 1.5, 'Color', colors(i, :), ...
...
legendEntries = {'27 years old', '39 years old', '47 years old', '70 years old', '74 years old', '87 years old'}; % Adjust according to numFiles
set(gca, 'XScale', 'log');
ylim([0, 19.5]);
